...
- **Surfaces.** A mismatch shows as an amber marker on the shared device row (Dashboard "My Devices" card and the user-details modal), a section on `/tasks`, and a count folded into the nav task badge and the Dashboard pending-tasks banner — all derived at read time from the same rule, so they cannot disagree.
- **Email (Phase 2.5).** Edge-triggered: one `callsign_mismatch_notice` email per breakage episode, sent on the confirmed good→bad transition (debounced across two consecutive polls), never repeated while the mismatch persists, re-armed once corrected. Sent post-commit and best-effort; `callsign_violation_first_seen_at` drives the debounce and `callsign_violation_notified_at` is the once-per-episode latch. Because the source is the live subscription table, the check is inherently connected-only.
