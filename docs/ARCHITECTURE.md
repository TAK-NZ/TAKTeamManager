...
- `authentikSync.startPeriodicSync()` — an interval-based user sync from Authentik (floor-clamped to 1 minute), which also runs the Reconciliation_Sweep. Gated by `AUTHENTIK_SYNC_ENABLED` (an opt-OUT kill-switch: on by default, disabled only when the value is exactly `'false'`) — used to halt the sync/sweep during a bulk import or Authentik maintenance. See `docs/authentik-scaling-lessons.md`.
...
- **`AdminCredentialRefreshJob`**, **`SubscriptionPoller`**, **`DeviceSync`**, **`CallsignPoller`** — all started only inside an `if (isDeviceMgmtEnabled())` block, all sharing the single `TakServerService` instance (via `AdminCredentialLoader`) so a refreshed Admin_Credential propagates to every one without a restart. `SubscriptionPoller` (history, `/Marti/api/clientEndPoints`, default 5-minute cadence) owns `last_seen_at`/`connected`; `CallsignPoller` is a separate, faster job (`/Marti/api/subscriptions/all`, `CALLSIGN_POLL_INTERVAL_SECONDS`, default 60s, floored at 60) that detects a device connected under a callsign not matching the one TAK Team Manager assigned, drives the in-app nudge, and sends the single first-detection email (see "Callsign Mismatch Detection" below).
...
- **`tak_devices`** — one TAK Server client certificate per row, keyed by `client_uid` (not a surrogate id). Columns: `user_id` (FK, `ON DELETE SET NULL`), `cert_id`, `issued_at`/`expires_at`, `last_seen_at`, `last_polled_at`, `revoked`, `connected`, and (callsign-mismatch detection) `observed_callsign`, `callsign_violation_first_seen_at`, `callsign_violation_notified_at`. Single-writer discipline per column is structural: `SubscriptionPoller` owns `last_seen_at`/`connected`; `CallsignPoller` owns `observed_callsign`/`callsign_violation_first_seen_at`/`callsign_violation_notified_at`; the revoke handler owns `revoked`; `DeviceSync`'s upsert must never touch any of them. A revoked row is never `connected` — `SubscriptionPoller` writes `connected = (reported AND revoked = false)` and the revoke handler clears `connected` when it sets `revoked`, so a revoked device never presents as live.
...
- **`user_cache`** — a denormalized read model synced from Authentik: `tak_role`, `tak_color`, `tak_callsign`, `groups`, `is_admin`, `is_team_device`, `device_label`, `callsign_suffix`, `email` (nullable, same invariant as `users.email`), and `last_login` (a mutable mirror of Authentik's `last_login`, refreshed every sync). `GET /api/users` and `GET /api/devices` are sourced from this local read model (joined to `users`), NOT a live per-request Authentik fetch — so listing, search, directory-scope narrowing, pagination, and the team/alphabet filters all run in SQL with an exact local total, and the pages stay up when Authentik is unavailable. The trade-off: an out-of-band Authentik-only create appears only after the next sync.
...
- **Orphaning** is automatic, never admin-triggered: `authentikSync.js`'s Reconciliation_Sweep, run once per successful periodic sync, sets `account_status='orphaned'` on any local row whose Authentik identity no longer exists, revoking its certificates and clearing its cached TAK attributes along the way. An orphaned account can be neither suspended nor unsuspended (there is no Authentik identity left to lock or unlock), and its row-level actions disappear from the client accordingly. The sweep runs ONLY when the periodic user fetch is provably complete: it compares the accumulated user count against the `pagination.count` Authentik reports and **skips the sweep entirely on any incomplete fetch** (recording the skip on `sync_status`), because trusting a partial fetch would false-orphan every real user it happened not to see — a real incident, see `docs/authentik-scaling-lessons.md`. Orphaning is a LOCAL status flag only; the federated Authentik identity is never deleted, so a false orphan is fully recoverable.
...
- **`CallsignPoller`** is a third scheduled job in the sync worker, backing Callsign Mismatch Detection (below).

Device lifecycle: enrollment (a certificate is issued; `DeviceSync` creates the `tak_devices` row) → periodic polling (`SubscriptionPoller` updates `connected`/`last_seen_at`; `CallsignPoller` updates `observed_callsign`) → revocation (an enqueued sync operation is processed by the worker's `revokeTakCertificates` handler; `connected` is cleared and never re-set while `revoked`) or renewal (a new certificate is minted and the prior one is superseded — see "Certificate Expiry Notifications" above).

### Callsign Mismatch Detection

Detects when a TAK client connects under a callsign that does not preserve the callsign TAK Team Manager assigned to that user, so the user can be nudged to correct it. Part of Device Management (same `DEVICE_MGMT_ENABLED` gate); time-based escalation and eventual revocation are explicitly out of scope for now.

- **Assigned vs observed.** The assigned callsign is `computeCallsignAttributes`'s assembled value (`user_cache.tak_callsign`): a TAM-determined `Organisation[-Team]` prefix plus the user-editable Name segment (`users.callsign_suffix`). `CallsignPoller` reads the *observed* callsign from TAK Server's live subscription table (`/Marti/api/subscriptions/all`, joined to a `tak_devices` row by `client_uid`) into `observed_callsign`.
- **The rule** (`server/utils/callsignMatch.js`, pure/property-tested): the observed callsign is acceptable iff it equals the assigned one or extends it past a non-alphanumeric boundary (the user may *append* — e.g. `FENZ-STL-J.Doe (Tablet)` — but not alter the assigned part). CloudTAK connections (`ANDROID-CloudTAK-` cert prefix), teamless users (no assigned callsign), and revoked devices are never flagged.
- **Surfaces.** A mismatch shows as an amber marker on the shared device row (Dashboard "My Devices" card and the user-details modal), a section on `/tasks`, and a count folded into the nav task badge and the Dashboard pending-tasks banner — all applying the same `callsignMatch.js` rule against the same stored values, so they cannot disagree on *what* counts as a mismatch, and (see "Standing task, not live status" below) they now agree on *when* it stops showing too.
- **Standing task, not live status.** Both the `/tasks` section and the device-row marker are scoped to a *standing, unresolved misconfiguration* rather than a live connection: a callsign mismatch is a persistent client-side setting that disconnecting does not fix, so the flag deliberately **outlives** the connection. The earlier `connected`-gated behaviour made both surfaces evaporate the moment the offending device went offline — precisely when the user stops looking — even though nothing was corrected. The shared scope is an *open, debounced episode* (`callsign_violation_first_seen_at IS NOT NULL` — set by `CallsignPoller` after two consecutive mismatching polls, cleared the instant it observes a correction, so a fixed-then-disconnected device drops off immediately) that was *recently seen* (`last_seen_at` within `CALLSIGN_MISMATCH_STALE_DAYS`, default 7, a NULL `last_seen_at` treated as in-window; beyond the window the flag ages out, since a device untouched that long is weak evidence the mismatch is still live). The two surfaces reach that scope differently: `listOwnCallsignMismatches` (the `/tasks` query) filters on the episode latch and the window in SQL, while `mapDevice` (the device-row flag, feeding the two device-*list* endpoints) selects `callsign_violation_first_seen_at` and `last_seen_at` as plain columns and computes the staleness verdict in JS — because a device *read* is held to exactly one visibility mechanism, row presence, and must grow no `NOW()`/interval comparison (Requirements 17.8, 21.9, structurally guarded). Both keep the shared CloudTAK/teamless/revoked exclusions and the `isCallsignAcceptable` verdict. `CALLSIGN_MISMATCH_STALE_DAYS` is server-only (it filters/decides, unlike the presentation-only `DEVICE_MGMT_EXPIRY_WARNING_DAYS`) and never surfaced through `GET /api/config/public`. The revoke-path row lookups (`findDeviceRow`) join in neither the assigned callsign nor the latch, so they always report `callsignMismatch: false` — the flag is a property of the list views, not of a single-device revoke resolution.
- **Email (Phase 2.5).** Edge-triggered: one `callsign_mismatch_notice` email per breakage episode, sent on the confirmed good→bad transition (debounced across two consecutive polls), never repeated while the mismatch persists, re-armed once corrected. Sent post-commit and best-effort; `callsign_violation_first_seen_at` drives the debounce and `callsign_violation_notified_at` is the once-per-episode latch. The email alone stays inherently connected-only — its good→bad *transition* can only be observed while the client is live in the subscription table — whereas the `/tasks` section and the device-row marker are both standing surfaces that persist past disconnect, as above.
