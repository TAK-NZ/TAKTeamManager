...
- A user's cached `tak_callsign`/`tak_color` (`user_cache`, and mirrored into Authentik's `takCallsign`/`takColor` attributes) must be cleared to ABSENT — via `UserAttributesService.clearTeamAttributes` — the moment they end up with ZERO `team_memberships` rows. ABSENT means: the `takCallsign`/`takColor` keys are DELETED from the Authentik `attributes` dict (pass `null` to `updateUserAttributes`, which deletes the key), and the `user_cache.tak_callsign`/`tak_color` columns are set to SQL `NULL`. NEVER the literal `'None'` and never `''` — `takColor: 'None'` is not a valid `TAK_Color` and leaks into TAK clients (see product.md's non-negotiable and the ABSENT-not-'None' rule on `updateUserAttributes`).
  ...
  - The periodic sync's Reconciliation_Sweep also calls `clearTeamAttributes`, but ONLY for an orphan candidate with zero DIRECT `team_memberships` rows — a false orphan of a still-teamed member must NOT have its callsign/colour wiped (`server/services/authentikSync.js`). The sync push block likewise DELETES the Authentik key (never pushes `''`/`'None'`) when the local cache value is absent.
