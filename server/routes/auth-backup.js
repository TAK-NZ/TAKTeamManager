...
router.get('/silent', (req, res) => {
  const authURL = `${process.env.AUTHENTIK_URL}/application/o/authorize/` +
    ...
    `&scope=openid profile email` +
    `&prompt=none`; // This makes it silent - no user interaction
...
router.get('/silent-callback', async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.send(`
        <script>
          window.parent.postMessage({ success: false }, 'http://44.229.3.37:5173');
        </script>
      `);
    }
    ...
    // Exchange code for token (same as regular callback)
    ...
    // Get user groups from Authentik API
    const userGroupsResponse = await axios.get(`${process.env.AUTHENTIK_URL}/api/v3/core/users/${userResponse.data.sub}/`, {
      headers: { Authorization: `Bearer ${process.env.AUTHENTIK_ADMIN_TOKEN}` }
    });

    const user = {
      ...userResponse.data,
      groups: userGroupsResponse.data.groups || [],
      groups_obj: userGroupsResponse.data.groups_obj || []
    };
    ...
    res.send(`
      <script>
        window.parent.postMessage({ 
          success: true, 
          redirectUrl: '${process.env.FRONTEND_URL}/dashboard?token=${jwtToken}'
        }, 'http://44.229.3.37:5173');
      </script>
    `);
  } catch (error) {
    console.error('Silent auth error:', error.response?.data || error.message);
    res.send(`
      <script>
        window.parent.postMessage({ success: false }, 'http://44.229.3.37:5173');
      </script>
