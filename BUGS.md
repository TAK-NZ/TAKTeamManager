...
- **Actual:** Worse than just "missing" — `Admin.jsx`'s "Color Mappings"/"Role Descriptions" tabs looked functional but were not. Sub-point status re-verified against code (the original description had gone partly stale):
  1. **Fixed.** `handleSaveColor`/`handleSaveRole` previously only updated local React state — both were non-`async`, contained no `await`, and touched no API wrapper at all, despite being wired to the visible save buttons. Both are now `async` and `await settingsAPI.updateTakMappings({ [configKey]: value })` (`PUT /api/settings/tak-mappings`) before updating local state, mirroring `handleSaveConfig`'s existing `await configAPI.update` pattern in the same file. The `config_key` for a given table row's display label (e.g. `'Yellow'` → `'tak_color_yellow'`) is resolved via the new `client/src/utils/takMappingKeys.js` (`colorLabelToConfigKey`/`roleLabelToConfigKey`), the client-side mirror of `settings.js`'s `COLOR_KEY_LABELS`/`ROLE_KEY_LABELS`. A failed save now surfaces an inline error (`mappingSaveError`) and leaves the edit-in-progress state untouched so the operator's unsaved edit stays visible and retryable, instead of silently reverting.
  2. **Fixed, for `Admin.jsx` specifically.** `Admin.jsx` previously called `configAPI.getColorMappings()` → `GET /api/config/color-mappings`, whose handler (`server/routes/config.js:33`) builds its whole response from `process.env.TAK_COLOR_*`/`TAK_ROLE_*` and has no PUT counterpart at all — which is WHY point 1's save could never have persisted regardless of whether it called an API: there was no writable endpoint on the other end of that read. `Admin.jsx` now reads via the new `settingsAPI.getTakMappings()` → `GET /api/settings/tak-mappings`, which reads the same `{colorMappings, roleDescriptions}` shape from the `system_config` rows the PUT above actually writes, so a save now round-trips correctly. `Dashboard.jsx`, `Teams.jsx`, and `TeamDetail.jsx` still read the legacy env-backed `GET /api/config/color-mappings` for their own (read-only, display-purposes) use — left as-is since those three never write to it and are outside this bug's scope, but worth a follow-up to point them at the database-backed endpoint too so a value edited in `/admin` is reflected everywhere immediately rather than only after the next deploy re-reads the env var.
  3. **Still fixed** — the dead `localStorage`/`Authorization: Bearer` code in `Dashboard.jsx`/`Admin.jsx` was replaced with the shared `configAPI`/`usersAPI`/`channelsAPI`/`syncAPI` wrappers, which also fixed `Admin.jsx`'s stats cards counting a paginated array's `.length` (capped at 50) instead of `pagination.total`. See NOTE-002 for what remains; full detail in git history.
...
- **Severity:** was blocker (silent data-loss UX on the color/role tabs); sub-point 4's missing branding/TAK-Server UI remains open as a separate, lower-severity gap.
- **Status:** partially fixed — sub-points 1-3 fixed (this was the actual reported bug: Colour Mappings/Role Descriptions edits now persist). Sub-point 4 (branding/TAK Server credential UI) remains open.
